...
bool RedisClient::connectToServer()
{
    ...
    std::memset(&hints, 0, sizeof(hints)); // Initializing a struct to contain 0 instead of garbage value because of auto initialization
    hints.ai_family = AF_UNSPEC;           // Address family unspecified, meaning it can be IPv4 or IPv6
    hints.ai_socktype = SOCK_STREAM;       // Socket type to TCP

    std::string portStr = std::to_string(port);
    int err = getaddrinfo(host.c_str(), portStr.c_str(), &hints, &res); // creating a linked list of the address info structure

    if (err != 0) // if there are any erros in address resolution
    {
